...
class GraphEditor extends Component {
  ...
  componentDidMount() {
    ...
    d3.select(".input-number-increment").on("click", incrementTextSize);
    d3.select(".input-number-decrement").on("click", decrementTextSize);
...
*/
    ...
    function togglePreview() {
      ...
      if (previousPreview === false) {
        setError("Preview Mode Enabled", 1000);
        ...
        setError("Preview Mode Disabled", 1000);
        that.setState({ preview: !that.state.preview });
    ...
    function incrementTextSize() {
      if (
        that.selectedNode &&
        that.selectedNode.type === "text" &&
        that.selectedNode.textSize === 30
      ) {
        return;
      }

      if (that.selectedNode && that.selectedNode.type === "text") {
        // Changes Data
        that.selectedNode.textSize++;

        // Changes D3's DOM (restart incorporates textSize updating)
        restart();
    ...
    function decrementTextSize() {
      if (
        that.selectedNode &&
        that.selectedNode.type === "text" &&
        that.selectedNode.textSize === 10
      ) {
        return;
      }
      if (that.selectedNode && that.selectedNode.type === "text") {
    ...
    function setError(error, waitTime) {
      console.log("setted");
      that.setState(
        {
          errMsg: error
        ...
        function() {
          ...
          setTimeout(function() {
            d3.select(".errMsginner")
              ...
              .style("opacity", 0)
              .on("start", () => {
                that.isErroring = true;
              })
              .on("end", function() {
                that.setState({ errMsg: "" });
                that.isErroring = false;
              });
            //  that.setState({ errMsg: "" });
          }, waitTime);
        }
    ...
    function invertOptionGroupVisibility() {
      ...
      optionGroup
        ...
        .attr("transform", function(d) {
          ...
          } else if (that.selectedNode && that.selectedNode.type === "text") {
            return getTranslateString(
    ...
    function undo() {
      if (that.historyStep !== 0) {
        ...
        switch (inverseCommand.type) {
          case "delNode":
            if (that.selectedNode.id === inverseCommand.node.id) {
              that.selectedNode = null;
              that.forceUpdate();
            }
      ...
      <React.Fragment>
        <div id="editorsContainer" className="">
          ...
          <ColorCard
            ...
            type={this.selectedNode ? this.selectedNode.type : null}
            textSize={
