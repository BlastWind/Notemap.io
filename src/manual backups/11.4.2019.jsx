...
class GraphEditor extends Component {
    ...
    this.nodes = [
      {
        ...
        width: 50,
        height: 40,
        text: ["I hope you are having a", "superb day"],
        opacity: 1,
        x: 750,
        y: 200
      },
      {
        ...
        width: 50,
        height: 40,
        text: ["hi my name is", "andrew chen"],
        opacity: 1
      }
    ];
    this.lastNodeId = 2;
  ...
  */
          ...
          var paragraph = textBox
            .append("xhtml:p")
            .html("<p>this is a<br>placeholder<br>go crazy go stupid</p>")
          ...
          paragraph
            ...
            .on("keydown", function() {
              if (d3.event.keyCode === 13 && !d3.event.shiftKey) {
                ...
                isShiftEntered = false;
              } else if (d3.event.keyCode === 13 && d3.event.shiftKey) {
                //keydown detects if both shift + enter
                //in keyup this is impossible because you shift & enter are two events
                isShiftEntered = true;
              } else {
                isShiftEntered = false;
              }
            ...
            .on("keyup", function() {
              ...
              if (d3.event.keyCode === 13 && !d3.event.shiftKey) {
                //if only enter is released
                this.blur();
              } else {
                //if any other key is released
                var textContent = this.innerText;
                if (that.textInputCircle)
                  that.nodes.map(eachNode => {
                    if (eachNode.id === that.textInputCircle.id) {
                      if (isShiftEntered) {
                        //if shift enter was released
                        console.log(d3.select(this)._groups[0][0].childNodes);

                        //eachNode.text = textContent + "/n/";
                      } else {
                        //if any other text was entered
                        console.log("not shift entered");
                        console.log(eachNode.text);
    ...
    function huh() {
      ...
      if (that.isCentering) {
        ...
        force.force("x", null).force("y", null);
      }
    ...
    function keydown() {
      if (d3.event.ctrlKey) {
        linkPicture.attr("src", link_purple);
        return;
      }
